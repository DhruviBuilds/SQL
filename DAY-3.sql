...
--1 Arrange the records by unit_price from highest to lowest and, for transactions having the same unit price, arrange them by quantity from highest to lowest.
SELECT * 
FROM 
sales_transactions 
ORDER BY unit_price,quantity DESC;

/*2 Identify transactions where the unit price is greater than 50000 and the quantity purchased is greater than 1.
...
/* 3 Identify transactions made by Premium customers where the unit price is greater than 25000.
...
/* 4Identify transactions where the discount percentage is greater than 15 and the quantity purchased is at least 3.
...
FROM
sales_transactions
WHERE city IN('Ahmedabad','Delhi','Mumbai') AND (unit_price > 20000)
ORDER BY city,unit_price DESC;

/* 6 Identify transactions where the payment mode is either Online or Card, the quantity is greater than 2, and the unit price is above 15000.
...
/* 7 Identify transactions belonging to either:
...
/* 8 Identify transactions made by customers who are either Premium or Regular, where the unit price is greater than 30000 and the quantity is greater than 1.
...
SELECT customer_name,product_name,quantity,unit_price,customer_type
FROM
sales_transactions
WHERE (customer_type = 'Premium' OR customer_type = 'Regular') AND (quantity > 1) AND (unit_price > 30000)
ORDER BY unit_price DESC;

/* 9 Identify transactions handled by salespersons other than a selected salesperson, where the quantity is greater than 4 and the discount percentage is less than 20.
...
FROM 
sales_transactions
WHERE (salesperson <> 'Neha') AND (quantity > 4) AND (discount_percent < 20)
ORDER BY quantity DESC;

/* 10 Identify transactions satisfying either of the following business conditions:
...
SELECT customer_name,transaction_id,product_name,quantity,unit_price,category
FROM
sales_transactions
WHERE (unit_price > 10000 AND quantity > 5) OR (unit_price > 10000 AND (quantity BETWEEN 2 AND 5))
ORDER BY unit_price DESC;

/* 11 Identify transactions from cities other than Ahmedabad where:
...
FROM 
sales_transactions
WHERE (payment_mode <> 'Cash') AND (quantity > 2) AND (unit_price > 20000)
ORDER BY unit_price DESC;

/* 12 Identify Electronics transactions where:
...
FROM
sales_transactions
WHERE (unit_price > 40000) AND (discount_percent < 15) AND (quantity > 1)
ORDER BY unit_price DESC;

/* 13 Identify Furniture transactions where the quantity is greater than 3 or the unit price is greater than 25000.
...
/* 14 Identify transactions where:
...
FROM 
sales_transactions
WHERE (customer_type = 'Premium') AND (quantity > 1) AND (unit_price > 20000) AND (payment_mode <> 'Cash')
ORDER BY unit_price DESC;

/* 15 Identify transactions where the unit price is greater than 50000 and the discount percentage is greater than 10.
...
SELECT customer_name,customer_type,product_name,unit_price,quantity,payment_mode,city
FROM
sales_transactions
WHERE ((customer_type = 'VIP' OR customer_type = 'Premium') AND (city <> 'Ahmedabad')) OR ((quantity > 3) AND (unit_price > 60000))
ORDER BY unit_price DESC;
...
FROM 
sales_transactions
WHERE ((unit_price < 50000) AND (quantity > 2) AND (discount_percent > 20) AND (city <> 'Mumbai') AND (payment_mode <> 'Cash'))
ORDER BY discount_percent DESC;
...
FROM 
sales_transactions
WHERE ((unit_price > 40000) AND (customer_type = 'Premium') AND (category = 'Electronics') AND (payment_mode <> 'Cash')) OR
((customer_type = 'VIP') AND (unit_price > 50000) AND (payment_mode <> 'Cash')) OR
((customer_type = 'Regular') AND (unit_price > 10000) AND (quantity > 5) AND (payment_mode <> 'Cash'))
ORDER BY unit_price DESC;
...
FROM 
sales_transactions
WHERE ((customer_type = 'Premium') AND (category = 'Electronics') AND (unit_price > 35000) AND (payment_mode <> 'Cash') AND (city <> 'Ahmedabad') AND (discount_percent <= 25)) OR
((customer_type = 'VIP') AND (category = 'Furniture') AND (quantity > 2) AND (payment_mode <> 'Cash') AND (city <> 'Ahmedabad') AND (discount_percent <= 25)) OR
((customer_type = 'Regular')  AND (unit_price > 75000) AND (payment_mode <> 'Cash') AND (city <> 'Ahmedabad') AND (discount_percent <= 25))
